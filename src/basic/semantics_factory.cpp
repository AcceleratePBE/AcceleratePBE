...
WitnessList CircuitBinary::witnessFunction(const DataList &oup, GlobalInfo *global_info) {
    if (oup.size() == 0) {
        return {{{}, {}}};
    }
    assert(oup.size() == 1);
    ...
    for (int a = 0; a < 2; ++a) {
        for (int b = 0; b < 2; ++b) {
            ...
            Data run_result = run(input, global_info);
...
WitnessList CircuitNot::witnessFunction(const DataList &oup, GlobalInfo *global_info) {
    if (oup.size() == 0) {
        return {{{}}};
    } else {
        assert(oup.size() == 1);
...
WitnessList CircuitOr::witnessFunction(const DataList &oup, GlobalInfo *global_info) {
    if (oup.size() == 0) {
        return {{{}, {}}};
    }
    assert(oup.size() == 1);
...
WitnessList CircuitAnd::witnessFunction(const DataList &oup, GlobalInfo *global_info) {
    if (oup.size() == 0) {
        return {{{}, {}}};
    }
    assert(oup.size() == 1);
