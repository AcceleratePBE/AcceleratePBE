...
z3::expr util::defineVar(z3::context& ctx, Type type, std::string name) {
    switch (type) {
...
Data util::parseDataFromModel(z3::context &ctx, z3::model& model, Type type, std::string name) {
    z3::expr var = util::defineVar(ctx, type, name);
    ...
    switch (type) {
        ...
        case TBOOL: {
            if (value.is_bool()) {
                ...
                assert(var.to_string() == value.to_string());
                return Data(new BoolValue(false));
            }
        }
        default:
            assert(0);
