...
def check_valid(rule):
    ...
    if type(rule) == tuple:
        if rule[0] == "Int" and abs(rule[1]) > 1 and not is_for_blaze: return False
    return True
...
def formalize_bmExpr(bm_expr):
    ...
    example_list = getSublist(bm_expr, "constraint")
    if is_get_invalid_list_for_blaze:
        is_valid = True
        for example in example_list:
            ...
            if type(I) == tuple:
                I, O = O, I
            assert type(I) == list and type(O) == tuple
            if O[0] != "String":
                is_valid = False
            for inp_value in I[1:]:
                if inp_value[0] != "String":
                    is_valid = False
        if not is_valid:
            with open("invalid_list_for_blaze", "a") as oup:
                oup.write(file_name.split('/')[-1] + "\n")

    if is_parse_for_blaze:
        with open(blaze_file, "w") as oup:
            oup.write("// examples \n")
            for example in example_list:
                inp = example[1][1]
                oup_value = example[1][2]
                if type(inp) == tuple:
                    inp, oup_value = oup_value, inp
                assert type(inp) == list and type(oup_value) == tuple
                input_list = []
                for inp_value in inp[1:]:
                    assert type(inp_value) == tuple
                    input_list.append(str(inp_value[1]))
                oup.write("\"" + "|".join(input_list) + "\"" + "\n")
                oup.write("\"" + str(oup_value[1]) + "\"\n")
            oup.write("\n")
            oup.write("// extra constants \n")
            oup.write("s:" + "|".join(list(map(lambda x: "\"" + x +"\"#10", string_const))))

    return result

if __name__ == "__main__":
    if len(sys.argv) not in [4, 5]:
        print("python3 main.py $file_name $benchmark_type $output_file")
        exit(0)
    file_name = sys.argv[1]
    benchmark_name = sys.argv[2]
    blaze_file = "benchmark_for_blaze/" + file_name.split("/")[-1].split(".")[0]
