...
namespace {
    ...
    void checkWitness(Semantics* semantics, const WitnessList& result, const DataList& oup, GlobalInfo* info) {
#ifndef DEBUG
        assert(0);
#endif
        for (auto& term: result) {
            bool flag = true;
            DataList inp;
            for (auto& param_value: term) {
                if (param_value.size() != 1) {
                    flag = false;
                    break;
                }
                inp.push_back(param_value[0]);
            }
            if (!flag) continue;
            if (!util::checkInOupList(semantics->run(inp, info), oup)) {
                std::cout << "Wrong " << std::endl;
                std::cout << semantics->name << " " << util::dataList2String(oup) << " " << util::dataList2String(inp) << " => " << semantics->run(inp, info).toString() << std::endl;
                assert(0);
            }
        }
    }
...
bool SynthesisTask::getBestProgramWithoutHeuristic(SynthesisTask::VSANode *node, int example_id, double limit) {
    if (node->best_program != nullptr) return true;
    if (TRIVIAL_BOUND(node) < limit) return false;
    if (node->l != nullptr && config::KIsUseMultiExample) {
        if (!getBestProgramWithoutHeuristic(node->l, example_id - 1, limit) || !getBestProgramWithoutHeuristic(node->r, -example_id, limit)) {
            node->updateP();
            return false;
        }
    ...
    while (true) {
        ...
        for (auto *edge: possible_edge) {
            double edge_w = edge->rule_w;
            for (auto* sub_node: edge->v) edge_w += TRIVIAL_BOUND(sub_node);
            if (edge_w <= limit) continue;
            int unfinished_num = 0;
            for (auto *sub_node: edge->v) {
                if (sub_node->best_program == nullptr) ++unfinished_num;
            }
            if (unfinished_num == 0) continue;
            if ((limit - edge_w) / unfinished_num < best_remain) {
                best_remain = (limit - edge_w) / unfinished_num;
                best_edge = edge;
            }
        }
        ...
        for (auto* sub_node: best_edge->v) {
            remain_list.push_back(TRIVIAL_BOUND(sub_node));
        }
        bool is_progress = false;
        for (int i = 0; i < remain_list.size(); ++i) {
            VSANode* sub_node = best_edge->v[i];
            if (sub_node->best_program == nullptr && getBestProgramWithoutHeuristic(sub_node, example_id,remain_list[i] + best_remain)) {
                is_progress = true;
                break;
            }
        }
        if (!is_progress) possible_edge.erase(std::find(possible_edge.begin(), possible_edge.end(), best_edge));
        int now = 0;
        node->p = limit;
        for (auto* edge: possible_edge) {
            edge->updateW();
    ...
    node->updateP();
    Program* best_program = nullptr;
    double best_probability = -1e100;
    for (auto* edge: node->edge_list) {
        bool is_finished = true;
        ...
        if (is_finished) {
            Program* candidate = new Program(sub_list, edge->semantics);
            double current_probability = calculateProbability(node->state, candidate);
            if (current_probability < limit) continue;
            if (best_program == nullptr || current_probability > best_probability) {
    ...
    if (best_program != nullptr) {
        node->best_program = best_program;
...
Program* SynthesisTask::synthesisProgramFromExample() {
    if (example_list.size() == 0) {
        return getBestProgramWithoutOup(0);
    } else {
        StateValue value;
        ...
        if (config::KIsUseHeuristic) {
            while (!getBestProgramWithOup(node, example_list.size() - 1, value_limit)) {
                value_limit -= 3;
                assert(value_limit >= -1000);
                std::cout << value_limit << std::endl;
            }
        } else {
            std::cout << "without" << std::endl;
            while (!getBestProgramWithoutHeuristic(node, example_list.size() - 1, value_limit)) {
                value_limit -= 3;
                assert(value_limit >= -1000);
                std::cout << value_limit << std::endl;
            }
...
Program * SynthesisTask::solve() {
    ...
    while (1) {
        ...
        printf("(%.3lf) %s\n", calculateProbability(0, result), result->toString().c_str());
        ...
        std::cout << "new example " << counter_example->toString() << std::endl;
    }
}